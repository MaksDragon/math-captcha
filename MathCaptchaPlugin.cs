...
public class MathCaptchaPlugin : TerrariaPlugin
{
    ...
    public override void Initialize()
    {
        ...
        // Регистрация базовой команды /captcha. Требует разрешение "captcha.admin"
    ...
    {
        // Переключаем состояние при каждом вызове команды /captcha
        ...
        else
        {
            ...
            // Очищаем активные сессии, чтобы никого не забанило по таймеру после отключения
            _sessions.Clear();
        }
    }
    ...
    {
        // Если капча отключена глобально — ничего не делаем
        if (!_isEnabled)
            return;
        ...
        // Инициализируем сессию (50 секунд на ответ, первое уведомление отправлено сейчас)
    ...
    private void OnUpdate(EventArgs args)
    {
        ...
        foreach (var entry in _sessions.ToList())
        {
            ...
            // Если игрок вышел с сервера, просто удаляем сессию без бана
            ...
            // Проверка на истечение общего времени (50 секунд)
            ...
            // Спам-механика: напоминаем каждые 10 секунд
            if ((now - session.LastNotificationTime).TotalSeconds >= 10)
            {
                ...
                session.LastNotificationTime = now; // Сбрасываем таймер напоминания
            }
        }
    }

    // Вспомогательный метод для красивого вывода капчи в чат
